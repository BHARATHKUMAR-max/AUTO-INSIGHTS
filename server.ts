...
const groqApiKey = process.env.GROQ_API_KEY?.replace(/^["']|["']$/g, '');
...
async function testDB() {
  ...
  const tables = ['datasets', 'profiles', 'analyses', 'chat_history'];
  
  for (const table of tables) {
    const { error } = await supabase.from(table).select('count', { count: 'exact', head: true });
    if (error) {
      console.error(`DB Connection Error for table '${table}':`, error.message);
      console.error(`IMPORTANT: Please ensure you have run the SQL migrations to create the '${table}' table.`);
    } else {
      console.log(`DB Connection Verified. '${table}' table is accessible.`);
    }
  }
  console.log("----------------------");
}
...
app.post("/api/chat", async (req, res) => {
  ...
  // Optimize profile to save tokens
  const optimizedProfile = {
    rows: profile?.row_count,
    cols: profile?.col_count,
    columns: Object.entries(profile?.columns || {}).reduce((acc: any, [name, stats]: [string, any]) => {
      acc[name] = {
        type: stats.type,
        unique: stats.unique_count,
        sample: stats.sample_values?.slice(0, 3)
      };
      return acc;
    }, {})
  };
  ...
  const systemPrompt = `
    ...
    DATA PROFILE:
    ${JSON.stringify(optimizedProfile)}
    ...
    3. If you don't know the answer, say you don't know based on the data.
  `;
  ...
  try {
    ...
    const completion = await groq.chat.completions.create({
      ...
      model: "llama-3.1-8b-instant", // Use 8b model for higher TPM limits on free tier
      stream: false
    });
...
// API: Generate Custom Graph Configuration
app.post("/api/generate-graph", async (req, res) => {
  ...
  const systemPrompt = `
    ...
    Return ONLY a JSON object:
    {
      ...
      "color": "column_name_for_grouping_or_segmentation_optional",
      "aggregation": "sum|avg|count|none",
      "reasoning": "Brief explanation",
      "python_code": "import plotly.express as px\n# ... generate code using df ..."
    }
  `;
